...
class RequestedWorkDetailsActivity : AppCompatActivity() {
        ...
        db.collection("work_posts").document(postId)
            ...
            .addOnSuccessListener { document ->
                ...
                if (document.exists()) {
                    ...
                    // CRITICAL: Ensure your Work Post document contains "clientId"
                    ...
                    // Fetch the client profile using the ID found in the work post
                    if (clientId != null) {
                        fetchClientProfile(clientId)
                    } else {
                        tvClientName?.text = "Client Unknown"
                    }
                }
            }
    }
    ...
    private fun fetchClientProfile(clientId: String) {
        // We look in the "users" collection where ClientRegisterActivity saved the data
        db.collection("users").document(clientId).get()
            .addOnSuccessListener { document ->
                if (isFinishing || isDestroyed) return@addOnSuccessListener

                if (document.exists()) {
                    // CHANGED: Using "name" because that is what you used in ClientRegisterActivity
                    val name = document.getString("name") ?: "Unknown Client"
                    ...
                    tvUserInitials?.text = if (initials.isNotEmpty()) initials else "?"
                }
            }
            .addOnFailureListener {
                tvClientName?.text = "Error loading name"
            }
    }
    ...
    private fun decodeAndShowImage(base64String: String) {
        try {
            ...
            Log.e("RequestedWorkDetails", "Image decode failed", e)
        }
    }
